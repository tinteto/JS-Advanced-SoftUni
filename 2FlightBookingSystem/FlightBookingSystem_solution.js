class FlightBookingSystem {
    ...
    addFlight(flightNumber, destination, departureTime, price) {
      ...
      //[{flightNumber: 'AA101', destination: 'Los Angeles', departureTime: '9:00AM', price: 250 }, {}, {}]
      return `Flight ${flightNumber} to ${destination} has been added to the system.`;
    }
  
    bookFlight(passengerName, flightNumber) { // Alice, AA101
      const flight = this.flights.find(f => f.flightNumber === flightNumber);
      // f.flightNumber - защото достъпвам масив от обекти, и flightNumber е property name в обект
      if (!flight) {
        return(`Flight ${flightNumber} is not available for booking.`);
      }
  
      this.bookings.push({ passengerName, flightNumber });
      // [{passengerName: 'Alice', flightNumber: 'AA101}, {}]
      this.bookingsCount++;
